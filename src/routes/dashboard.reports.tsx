...
function ReportsPage() {
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      const [residents, properties, invoices, payments, complaints, visitors, incidents] =
        await Promise.all([
      ...
      const outstanding = (invoices.data ?? []).reduce(
        (s, i) => s + (Number(i.amount) - Number(i.amount_paid ?? 0)),
        0,
      );
  ...
  if (isLoading) {
    return <PageLoading label="Preparing reports" onRetry={() => void refetch()} />;
  }
  if (isError) {
